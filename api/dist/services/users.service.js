...
var auth = function auth(email, password) {
  ...
  var authenticate = function authenticate() {
    _dbConnection.users.findOne({ email: email }, { created_at: 0, updated_at: 0 }, function (err, user) {
      _bcryptjs2.default.compare(password, user.hash).then(function (res) {
        if (user && res) {
          deferred.resolve({
            token: _jsonwebtoken2.default.sign({ sub: user._id }, secret),
            name: user.name,
            avatar: user.avatar,
            isAdmin: user.admin });
        }
  ...
  _dbConnection.users.findOne({ email: email }, function (err, user) {
    ...
    if (!user) deferred.reject('E-mail ' + email + ' not exist.');else authenticate();
  });
...
var list = function list() {
  ...
  _dbConnection.users.find({}, { hash: 0, email: 0, avatar: 0 }, function (err, users) {
    if (err) deferred.reject(err.name + ' : ' + err.message);
...
var getById = function getById(_id) {
  ...
  _dbConnection.users.findById(_id, { hash: 0, email: 0, avatar: 0 }, function (err, user) {
    if (err) deferred.reject(err.name + ' : ' + err.message);
...
var create = function create(userParam) {
  ...
  var createUser = function createUser() {
    ...
    var avatarRandom = function avatarRandom() {
      var ghIds = [342215, 5755568, 499550];
